...
exports.create          = function (req, res, next) {
    ...
    function createData (callback) {
        ...
        roleDAL.create(body,function (err, data) { // Creating role data
            queryResponseHandler(err,data,res,function (err, data) {
                if(data){ // role data successfully created
                    res.status(201);
                    res.json(data);
                    callback(null);
                }else {
                    let errMsg = errorCodes.SEC.NO_DATA_FOUND;
                    errMsg.detail = 'role data could not be found (Failed to create role data)';
                    res.status(500);
                    res.json(errMsg);
                }
            });
...
exports.find            = function (req, res, next) {
    ...
    if(req.query._id !== undefined) {
        ...
        let option = {
            ...
            limit    : req.query.limit === undefined ? config.COLLECTION_RETURN_SIZE : Number(req.query.limit),   // assigns default limit value (passed to the config), if not specified. // assigns default limit value (passed to the config), if not specified.
            select   : "name description members accessRoutes firstModified lastModified"
        };

        let query = controllerHelper.queryFilter(req,["name","description","members","accessRoutes" , "_id", "__v"]);
        // get collection paginated invoked
        roleDAL.getCollectionsPaginated(query,option,function (err,data) {
            queryResponseHandler(err,data,res,function (err, data) { // Possible errors are handled.
                if(data){ // Data found.
                    ...
                    let errMsg = errorCodes.SEC.NO_DATA_FOUND;
                    errMsg.detail = "No role data found."; // Adding detailed data.
                    res.status(404);
                    res.json(errMsg);
                }
            });
...
/**
 * @name                - Update Many
 * @description         - Updates all role data that matches query
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
exports.update          = function (req, res, next) {
    ...
    let body = req.body,
        query = controllerHelper.queryFilter(req,["name","description","members","accessRoutes", "_id", "__v"]);
    ...
    function pickUpdateData (callback)                  {
        ...
        controllerHelper.pickUpdateData(["name","description","members","accessRoutes"],req,function (err,validUpdateData) { // Picking up valid update data.
    ...
    function updateData (validUpdateData,callback)  {
        ...
        if(operation !== undefined && target !== undefined){ // Operation method is an array.
            if(Array.isArray(body)){
                if(operation === "pull"){
                    roleDAL.pullFromArray(query, target, body, function (err, data) {
                        if(!err){
                            res.status(200);
                            res.json(data);
                        }else{
                    ...
                    roleDAL.pushToArray(query, target, body, function (err, data) {
                        if(!err){
                            res.status(200);
                            res.json(data);
                        }else{
            ...
            if(Object.keys(query).length === 0){
                ...
                roleDAL.updateMany(query,validUpdateData,function (err,data) {   // handling update.
                    queryResponseHandler(err,data,res,function (err, data) {    // Possible errors are handled.
                        res.status(200);
                        res.send(data);
                        callback(null);
...
/**
 * @name                - Remove Many
 * @description         - Removes all role data that matches query
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
exports.remove          = function (req, res, next) {
    ...
    if(Object.keys(query).length === 0){
        ...
        roleDAL.removeMany(query,function (err,data) {   // handling update.
            queryResponseHandler(err,data,res,function (err, data) {   // Possible errors are handled.
                res.status(200);
                res.send(data);
            });
 ...
 *
 * @name                - Find public
 * @description         - Find role data by id visible only fields that are public.
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
function getPublic  (req, res, next) {
    ...
    let
        roleId   = req.query._id,
        query       = {_id: roleId}; // query construction.

    roleDAL.getPublic(query,function (err,data) { // retrieve role public data (with out the value)
        queryResponseHandler(err,data,res,function (err, data) { // Error handled.
            if(!data){ // No role value could be found
                res.status(404);
                res.json(errorCodes.SEC.NO_DATA_FOUND);
            }else if(data){ // Found role data
                res.status(200);
                res.send(data);
            }
        });
...
/**
 * @name                - Find private
 * @description         - Find role data by id visible only fields that are private.
 * @param req           - Request object
 * @param res           - Response object
 * @param next          - Next
 */
function getPrivate (req, res, next) {
    ...
    roleDAL.getPrivate(query,function (err,data) { // retrieve role public data (with out the value)
        queryResponseHandler(err,data,res,function (err, data) { // Error handled.
            if(!data){ // No role value could be found
                res.status(404);
                res.json(errorCodes.SEC.NO_DATA_FOUND);
